...
def main():
    ...
    parser.add_argument("-b", "--base", default=100, type=int)
    parser.add_argument("-md", "--max_distance", default=500.0, type=float)
    ...
    color_water_list = [(3, 207, 252), (153, 162, 157), (66, 165, 245)]
    # colors_list = [(129, 199, 132)]
    # color_water_list = [(66, 165, 245)]
    color_other_list = [(186, 104, 200)]
    ...
    for col in range(0, pil_image.size[0], planet_size):
        for row in range(0, pil_image.size[1], planet_size):
            color_water = random.choice(color_water_list)
            color_ground = random.choice(colors_list)
            # while color_ground is color_water:
            #     color_ground = random.choice(colors_list)
            other_color = 0
            # other_color = random.choice(color_other_list)
            ...
            for i in range(col, col + planet_size):
                for j in range(row, row + planet_size):
                    ...
                    distance_from_center = math.sqrt(
                        math.pow((i - (col + (col + planet_size)) / 2), 2) +
                        math.pow((j - (row + (row + planet_size)) / 2), 2)
                    )

                    # gradient_perc = distance_from_center / max_distance
                    # pixel_value -= math.pow(gradient_perc, 10)

                    if distance_from_center < max_distance:
                        if other_color == 1 and int(pixel_value * 100.0) > 25:
                            pixels[i, j] = other_color
                        elif int(pixel_value * 100.0) > 0.0:
    ...
    pil_image.save('dist/planet.png')
